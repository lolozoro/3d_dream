...
async def create_brainmap(data: BrainMapCreate, service: BrainMapService = Depends(get_service)):
    """创建新的3D脑图"""
    with StepTimer(logger, f"API create_brainmap | brainmap_id={data.brainmap_id}"):
...
async def get_brainmap_full(brainmap_id: str, service: BrainMapService = Depends(get_service)):
    """获取完整脑图数据（用于前端3D渲染）"""
    with StepTimer(logger, f"API get_brainmap_full | brainmap_id={brainmap_id}"):
...
async def get_graph_data(brainmap_id: str, service: BrainMapService = Depends(get_service)):
    ...
    with StepTimer(logger, f"API get_graph_data | brainmap_id={brainmap_id}"):
        data = await service.get_brainmap_full(brainmap_id)
        return GraphData(
            nodes=data["nodes"],
            edges=data["edges"],
...
async def delete_brainmap(brainmap_id: str, service: BrainMapService = Depends(get_service)):
    """删除整个脑图"""
    with StepTimer(logger, f"API delete_brainmap | brainmap_id={brainmap_id}"):
...
async def get_brainmap_stats(brainmap_id: str, service: BrainMapService = Depends(get_service)):
    """获取脑图统计信息"""
    with StepTimer(logger, f"API get_brainmap_stats | brainmap_id={brainmap_id}"):
...
async def create_node(brainmap_id: str, data: NodeCreate, service: BrainMapService = Depends(get_service)):
    """在脑图中创建新节点（支持3D坐标）"""
    with StepTimer(logger, f"API create_node | brainmap_id={brainmap_id}"):
...
async def list_nodes(
    ...
    """查询节点（支持3D包围盒过滤）"""
    with StepTimer(logger, f"API list_nodes | brainmap_id={brainmap_id} skip={skip} limit={limit}"):
        import json
        bbox_dict = None
        if bbox:
            try:
...
async def get_node(node_id: int, service: BrainMapService = Depends(get_service)):
    """获取单个节点详情"""
    with StepTimer(logger, f"API get_node | node_id={node_id}"):
...
async def update_node(node_id: int, data: NodeUpdate, service: BrainMapService = Depends(get_service)):
    """更新节点（支持3D坐标更新）"""
    with StepTimer(logger, f"API update_node | node_id={node_id}"):
...
async def delete_node(node_id: int, service: BrainMapService = Depends(get_service)):
    """删除节点（级联删除关联边）"""
    with StepTimer(logger, f"API delete_node | node_id={node_id}"):
...
async def create_edge(brainmap_id: str, data: EdgeCreate, service: BrainMapService = Depends(get_service)):
    """创建边（连接两个节点）"""
    with StepTimer(logger, f"API create_edge | brainmap_id={brainmap_id}"):
        if data.brainmap_id != brainmap_id:
            raise HTTPException(status_code=400, detail="brainmap_id mismatch")
...
async def list_edges(
    ...
    """查询边"""
    with StepTimer(logger, f"API list_edges | brainmap_id={brainmap_id} skip={skip} limit={limit}"):
...
async def get_edge(edge_id: int, service: BrainMapService = Depends(get_service)):
    """获取单条边详情"""
    with StepTimer(logger, f"API get_edge | edge_id={edge_id}"):
...
async def update_edge(edge_id: int, data: EdgeUpdate, service: BrainMapService = Depends(get_service)):
    """更新边"""
    with StepTimer(logger, f"API update_edge | edge_id={edge_id}"):
...
async def delete_edge(edge_id: int, service: BrainMapService = Depends(get_service)):
    """删除边"""
    with StepTimer(logger, f"API delete_edge | edge_id={edge_id}"):
...
async def spatial_search(
    ...
    """3D空间球体范围搜索节点"""
    with StepTimer(logger, f"API spatial_search | brainmap_id={brainmap_id} radius={radius}"):
...
async def get_neighbors(
    ...
    """获取节点的邻居节点"""
    with StepTimer(logger, f"API get_neighbors | brainmap_id={brainmap_id} node_id={query.node_id}"):
...
async def get_subgraph(
    ...
    """获取子图（以某节点为中心，多跳半径）"""
    with StepTimer(logger, f"API get_subgraph | brainmap_id={brainmap_id} center={req.center_node_id} radius={req.radius}"):
...
async def find_paths(
    ...
    """查找两节点之间的路径"""
    with StepTimer(logger, f"API find_paths | brainmap_id={brainmap_id} src={req.source_node_id} dst={req.target_node_id}"):
...
async def batch_create_nodes(
    ...
    """批量创建节点（用于导入或初始化3D脑图）"""
    with StepTimer(logger, f"API batch_create_nodes | brainmap_id={brainmap_id} count={len(nodes)}"):
...
async def batch_create_edges(
    ...
    """批量创建边"""
    with StepTimer(logger, f"API batch_create_edges | brainmap_id={brainmap_id} count={len(edges)}"):
