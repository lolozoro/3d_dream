...
class BrainMapService:
    ...
    async def create_brainmap(self, data: BrainMapCreate) -> Dict[str, Any]:
        ...
        with StepTimer(logger, f"Service create_brainmap | brainmap_id={data.brainmap_id}"):
            # 检查是否已存在
    ...
    async def list_brainmaps(
        ...
        """获取脑图列表"""
        with StepTimer(logger, f"Service list_brainmaps | keyword={keyword} skip={skip} limit={limit}"):
    ...
    async def get_brainmap_meta(self, brainmap_id: str) -> Optional[BrainMapMeta]:
        """获取脑图元数据"""
        with StepTimer(logger, f"Service get_brainmap_meta | brainmap_id={brainmap_id}"):
    ...
    async def update_brainmap(self, brainmap_id: str, data: BrainMapUpdate) -> Optional[BrainMapMeta]:
        """更新脑图元数据"""
        with StepTimer(logger, f"Service update_brainmap | brainmap_id={brainmap_id}"):
    ...
    async def get_brainmap_full(self, brainmap_id: str) -> Dict[str, Any]:
        ...
        with StepTimer(logger, f"Service get_brainmap_full | brainmap_id={brainmap_id}"):
    ...
    async def delete_brainmap(self, brainmap_id: str) -> bool:
        """删除整个脑图"""
        with StepTimer(logger, f"Service delete_brainmap | brainmap_id={brainmap_id}"):
    ...
    async def get_brainmap_stats(self, brainmap_id: str) -> BrainMapStats:
        """获取脑图统计信息"""
        with StepTimer(logger, f"Service get_brainmap_stats | brainmap_id={brainmap_id}"):
    ...
    async def create_node(self, data: NodeCreate) -> Node:
        with StepTimer(logger, f"Service create_node | brainmap_id={data.brainmap_id}"):
    ...
    async def get_node(self, node_id: int) -> Optional[Node]:
        with StepTimer(logger, f"Service get_node | node_id={node_id}"):
    ...
    async def update_node(self, node_id: int, data: NodeUpdate) -> Optional[Node]:
        with StepTimer(logger, f"Service update_node | node_id={node_id}"):
    ...
    async def delete_node(self, node_id: int) -> bool:
        with StepTimer(logger, f"Service delete_node | node_id={node_id}"):
            node = await self.get_node(node_id)
            if not node:
                return False
            # 级联删除关联的边
    ...
    async def list_nodes(
        ...
        with StepTimer(logger, f"Service list_nodes | brainmap_id={brainmap_id} skip={skip} limit={limit}"):
    ...
    async def search_nodes_global(
        ...
        """全局跨脑图搜索节点"""
        with StepTimer(logger, f"Service search_nodes_global | keyword={keyword}"):
            like_str = f"%{keyword}%"
    ...
    async def create_edge(self, data: EdgeCreate) -> Edge:
        with StepTimer(logger, f"Service create_edge | brainmap_id={data.brainmap_id}"):
    ...
    async def get_edge(self, edge_id: int) -> Optional[Edge]:
        with StepTimer(logger, f"Service get_edge | edge_id={edge_id}"):
    ...
    async def update_edge(self, edge_id: int, data: EdgeUpdate) -> Optional[Edge]:
        with StepTimer(logger, f"Service update_edge | edge_id={edge_id}"):
    ...
    async def delete_edge(self, edge_id: int) -> bool:
        with StepTimer(logger, f"Service delete_edge | edge_id={edge_id}"):
            edge = await self.get_edge(edge_id)
            if not edge:
                return False
    ...
    async def list_edges(
        ...
        with StepTimer(logger, f"Service list_edges | brainmap_id={brainmap_id} skip={skip} limit={limit}"):
    ...
    async def spatial_query(
        ...
        """3D 空间球体范围查询"""
        with StepTimer(logger, f"Service spatial_query | brainmap_id={brainmap_id} radius={radius}"):
            # 先通过包围盒粗略过滤，再精确计算距离
    ...
    async def find_neighbors(self, query: NeighborQuery) -> List[Node]:
        """查找节点的邻居"""
        with StepTimer(logger, f"Service find_neighbors | node_id={query.node_id} direction={query.direction}"):
    ...
    async def get_subgraph(self, req: SubGraphRequest) -> Dict[str, Any]:
        """获取以某节点为中心的子图（支持多跳）"""
        with StepTimer(logger, f"Service get_subgraph | center={req.center_node_id} radius={req.radius}"):
            # BFS 获取范围内的节点ID
    ...
    async def find_paths(self, req: PathFindRequest) -> PathFindResponse:
        """查找两节点之间的路径（BFS）"""
        with StepTimer(logger, f"Service find_paths | src={req.source_node_id} dst={req.target_node_id}"):
    ...
    async def batch_create_nodes(self, brainmap_id: str, nodes_data: List[Dict[str, Any]]) -> List[Node]:
        """批量创建节点"""
        with StepTimer(logger, f"Service batch_create_nodes | brainmap_id={brainmap_id} count={len(nodes_data)}"):
            nodes = []
    ...
    async def batch_create_edges(self, brainmap_id: str, edges_data: List[Dict[str, Any]]) -> List[Edge]:
        """批量创建边"""
        with StepTimer(logger, f"Service batch_create_edges | brainmap_id={brainmap_id} count={len(edges_data)}"):
            edges = []
    ...
    async def batch_update_nodes(self, req: BatchNodeUpdateRequest) -> List[Node]:
        """批量更新节点（主要用于保存3D布局）"""
        with StepTimer(logger, f"Service batch_update_nodes | count={len(req.updates)}"):
            updated = []
    ...
    async def export_brainmap(self, brainmap_id: str) -> Dict[str, Any]:
        """导出脑图为JSON"""
        with StepTimer(logger, f"Service export_brainmap | brainmap_id={brainmap_id}"):
    ...
    async def import_brainmap(
        ...
        """从JSON导入脑图"""
        with StepTimer(logger, f"Service import_brainmap | brainmap_id={brainmap_id} nodes={len(nodes_data)} edges={len(edges_data)} overwrite={overwrite}"):
            if overwrite:
